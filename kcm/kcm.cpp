...
void KCMNetworkmanagement::onRequestExportConnection(const QString &connectionPath)
{
    ...
    QString error;
    VpnUiPlugin *vpnPlugin =
        KServiceTypeTrader::createInstanceFromQuery<VpnUiPlugin>(QStringLiteral("PlasmaNetworkManagement/VpnUiPlugin"),
    ...
    if (!filename.isEmpty()) {
        ...
        for (const KService::Ptr &service : services) {
            ...
            if (vpnPlugin && vpnPlugin->supportedFileExtensions().contains(ext)) {
                qCDebug(PLASMA_NM) << "Found VPN plugin" << service->name()
                                   << ", type:" << service->property("X-NetworkManager-Services", QVariant::String).toString();
