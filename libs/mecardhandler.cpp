...
QCoro::Task<void> MeCardHandler::activateConnectionInternal()
{
    ...
    // TODO Pick the wifi device that has the corresonding access point.
    ...
    if (!wifiDevice) {
        qWarning() << "Failed to find wifi device";
        co_return;
    }

    if (existingConnection) {
        qDebug() << "Updating existing connection" << existingConnection->uuid() << existingConnection->path();
