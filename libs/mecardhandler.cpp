...
QCoro::Task<void> MeCardHandler::activateConnectionInternal()
{
    ...
    if (!wifiDevice) {
        qCWarning(PLASMA_NM_LIBS_LOG) << "Failed to find wifi device";
        co_return;
    }

    if (existingConnection) {
        qCDebug(PLASMA_NM_LIBS_LOG) << "Updating existing connection" << existingConnection->uuid() << existingConnection->path();
    ...
    qCDebug(PLASMA_NM_LIBS_LOG) << "Adding new connection";
